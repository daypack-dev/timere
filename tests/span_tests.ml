...
module Qc = struct
  let normalize_is_idempotent =
    QCheck.Test.make ~count:10_000 ~name:"normalize_is_idempotent" timestamp
  ...
  let sub_self =
    QCheck.Test.make ~count:10_000 ~name:"sub_self" timestamp (fun x ->
        Span.(equal zero (x - x)))

  let double_neg =
    QCheck.Test.make ~count:10_000 ~name:"double_neg" timestamp (fun x ->
        Span.(equal x (neg @@ neg x)))

  let neg =
    QCheck.Test.make ~count:10_000 ~name:"neg" timestamp (fun x ->
        Span.(equal x (neg (neg x - x) - x)))

  let abs =
    QCheck.Test.make ~count:10_000 ~name:"abs" timestamp (fun x ->
        Span.(equal (abs x) (abs (neg x))))

  let abs_is_non_neg =
    QCheck.Test.make ~count:10_000 ~name:"abs_is_non_neg" timestamp (fun x ->
        Span.(abs x >= zero))
  ...
  let add_identity =
    QCheck.Test.make ~count:10_000 ~name:"add_identity" timestamp (fun x ->
  ...
  let add_neg_self =
    QCheck.Test.make ~count:10_000 ~name:"add_neg_self" timestamp (fun x ->
        Span.(equal zero (x + neg x)))

  let neg_add_self =
    QCheck.Test.make ~count:10_000 ~name:"neg_add_self" timestamp (fun x ->
