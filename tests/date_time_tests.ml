...
module Alco = struct
  ...
  let of_iso8601_leap_second1 () =
    Alcotest.(check span_testable)
      ...
      (Time.Date_time'.make_exn ~tz:Time_zone.utc ~year:2020 ~month:`Jan ~day:1 ~hour:0 ~minute:0 ~second:60 ~ns:100_000_000 ()
       |> Time.Date_time'.to_timestamp_single
      )
  ...
  let of_iso8601_case0 () =
    Alcotest.(check span_testable)
      ...
      (Time.Date_time'.make_exn ~tz:Time_zone.utc ~year:2020 ~month:`Jan ~day:1 ~hour:23 ~minute:59 ~second:59 ~ns:999_999_999 ()
       |> Time.Date_time'.to_timestamp_single
      )
