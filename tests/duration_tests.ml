...
module Qc = struct
  let to_of_span =
    QCheck.Test.make ~count:100_000 ~name:"to_of_span" duration (fun duration ->
        Duration.equal duration
        (Duration.of_span @@ Duration.to_span duration)
      )

  let of_to_span =
    QCheck.Test.make ~count:100_000 ~name:"of_to_span" pos_timestamp (fun span ->
        Span.equal
        span
          (Duration.to_span @@ Duration.of_span span)
      )
