...
module Qc = struct
  ...
  let union_order_does_not_matter =
    QCheck.Test.make ~count:10_000 ~name:"union_order_does_not_matter" QCheck.(list time) (fun l1 ->
        let l2 = List.rev l1 in
        let l3 =
          let x =
            l1 |> List.to_seq |> OSeq.take_nth 2
            |> List.of_seq
          in
          let y =
            l1 |> List.to_seq |> OSeq.drop 1 |> OSeq.take_nth 2
            |> List.of_seq
          in
          x @ y
        in
  ...
  let inter_order_does_not_matter =
    QCheck.Test.make ~count:10_000 ~name:"inter_order_does_not_matter" QCheck.(list time) (fun l1 ->
        let l2 = List.rev l1 in
        let l3 =
          let x =
            l1 |> List.to_seq |> OSeq.take_nth 2
