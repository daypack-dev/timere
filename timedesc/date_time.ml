...
let min_of_local_date_time_result (r : 'a local_date_time_result) : 'a =
  match r with `Single x | `Ambiguous (x, _) -> x

let max_of_local_date_time_result (r : 'a local_date_time_result) : 'a =
...
type t = {
  ...
  time : Time.t;
  tz : Time_zone.t;
  offset_from_utc : Span.t local_date_time_result;
}
...
let to_timestamp (x : t) : timestamp local_date_time_result =
  let open Span in
...
let to_timestamp_float_s x : float local_date_time_result =
...
let compare_chrono_min (x : t) (y : t) : int =
  let x = min_of_local_date_time_result @@ to_timestamp x in
  let y = min_of_local_date_time_result @@ to_timestamp y in
  Span.compare x y

let compare_chrono_max (x : t) (y : t) : int =
  let x = max_of_local_date_time_result @@ to_timestamp x in
  let y = max_of_local_date_time_result @@ to_timestamp y in
  Span.compare x y
