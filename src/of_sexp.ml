    ...
    | `Atom "branching" :: l -> (
        ...
        let days, l =
          ...
          | (`List (`Atom "month_days" :: days)) :: l ->
            Time.Month_days (List.map (range_of_sexp ~f:int_of_sexp) days), l
          | (`List (`Atom "weekdays" :: days)) :: l ->
            Time.Weekdays (List.map (range_of_sexp ~f:weekday_of_sexp) days), l
          | _ -> Time.Month_days [], l
        in
        let hmss, l =
          match l with
          | (`List (`Atom "hmss" :: hmss)) :: l ->
            List.map (range_of_sexp ~f:(fun x ->
                match x with
                | `List [hour; minute; second] ->
                  Time.{ hour = int_of_sexp hour;
    ...
    | `List l -> (
        ...
        | `Atom "intervals" :: l ->
          ...
          |> List.map (fun x ->
              match x with
              | `List [ x; y ] ->
                (timestamp_of_sexp x, timestamp_of_sexp y)
              | _ -> raise Invalid_data
            )
