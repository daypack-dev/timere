...
module Range = struct
  ...
  let int_pair_of_timestamp_pair
    ...
    (Timedesc.Span.get_ns_offset x, Timedesc.Span.get_ns_offset y)
...
module Ranges = struct
  ...
  module Make (B : Range.B) : S with type t := B.t = struct
    ...
    let get_inc s = get_inc ~to_int ~of_int s

    let get_exc s = get_exc ~to_int ~of_int s
...
let pattern ?(years = []) ?(year_ranges = []) ?(months = [])
    ...
    ?(minute_ranges = []) ?(seconds = []) ?(second_ranges = []) ?(ns = [])
    ?(ns_ranges = []) () : t =
  let years =
  ...
  in
  match
    (years, months, month_days, weekdays, hours, minutes, seconds, ns, ns_ranges)
  with
  | [], [], [], [], [], [], [], [], [] -> All
  | _ ->
      if
