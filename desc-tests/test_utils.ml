...
let pos_timestamp_bound_gen bound =
  QCheck.Gen.(
    map
      ...
      (pair
         (int64_bound_gen (Int64.pred Timedesc.Timestamp.(get_s max_val)))
         (int_bound 1_000_000_000)))
...
let date_time_testable : (module Alcotest.TESTABLE with type t = Timedesc.t) =
  (module struct
    type t = Timedesc.t
...
let iso_week_date_gen : (int * int * Timedesc.weekday) QCheck.Gen.t =
  ...
  map3
    (fun year week weekday ->
      let week =
        Int64.to_int week
        mod Timedesc.Utils.week_count_of_iso_year ~year
        + 1
      in
      (year, week, weekday))
...
let iso_week_date =
  QCheck.make
    ~print:(fun (year, week, weekday) ->
      Printf.sprintf "%d-%02d-%s" year week
