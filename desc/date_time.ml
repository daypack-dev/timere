...
type t = {
  ...
  let to_timestamp_local (x : t) : Span.t =
    ...
    *)
    let days_from_epoch =
      Span.For_human'.make_exn ~days:(jd_of_ydoy ~year:x.date.year ~day_of_year:x.date.day_of_year - jd_of_epoch) ()
    in
    let time_of_day = Time.to_span x.time in
    if Span.(days_from_epoch < zero) then
      Span.(Constants.one_day - time_of_day)
    else
      time_of_day
    ...
    let hour, minute, second, ns =
      ...
      | `Neg ->
        let x = Span.(For_human'.view
                        (Constants.one_day - For_human'.make_exn ~hours:x.hours ~minutes:x.minutes ~seconds:x.seconds
                           ~ns:x.ns ()) )
        in
        (x.hours, x.minutes, x.seconds, x.ns)
    in
...
module ISO_ord_date_time = struct
  ...
  let make ?(tz = Time_zone_utils.get_local_tz_for_arg ()) ?ns
      ?s_frac ~year ~day_of_year ~hour ~minute ~second () : (t, error) result =
    ...
    | Ok date ->
      ...
      | Error e -> Error (e :> error)
      | Ok time ->
        of_dt_with_missing_tz_info
          ~tz { date; time; tz_info = dummy_tz_info }
  ...
  let make_unambiguous ?tz ?ns
      ?s_frac ~year ~day_of_year ~hour ~minute ~second ~tz_offset () =
    match Date.ISO_ord_date.make ~year ~day_of_year with
    | Error e -> Error (e :> error)
    | Ok date ->
      match Time.make ~hour ~minute ~second ?ns ?s_frac () with
  ...
  let make_unambiguous_exn ?tz ?ns ?s_frac ~year ~day_of_year ~hour ~minute ~second ~tz_offset () =
    match make_unambiguous ?tz ?ns ?s_frac ~year ~day_of_year ~hour ~minute ~second ~tz_offset () with
    | Ok x -> x
    | Error e -> raise (Error_exn e)
end
...
module Ymd_date_time = struct
  ...
  let make ?(tz = Time_zone_utils.get_local_tz_for_arg ()) ?ns
      ?s_frac ~year ~month ~day ~hour ~minute ~second () : (t, error) result =
  ...
  let make_unambiguous ?tz ?ns ?s_frac ~year ~month ~day ~hour
  ...
  let make_unambiguous_exn ?tz ?ns ?s_frac ~year ~month ~day ~hour ~minute ~second
      ~tz_offset () =
    match
...
module ISO_week_date_time = struct
  ...
  let make ?(tz = Time_zone_utils.get_local_tz_for_arg ()) ?ns
      ?s_frac ~iso_week_year ~week ~weekday ~hour ~minute ~second () : (t, error) result =
  ...
  let make_exn ?tz ?ns ?s_frac ~iso_week_year ~week ~weekday ~hour ~minute ~second () =
    match make ?tz ?ns ?s_frac ~iso_week_year ~week ~weekday ~hour ~minute ~second () with
    | Ok x -> x
    | Error e -> raise (Error_exn e)
end
