...
module Odt' = struct
  ...
  let weekday (x : t) =
    let month, day = md_of_ydoy ~year:x.year ~day_of_year:x.day_of_year in
    weekday_of_ymd ~year:x.year ~month ~day

  let iso_week_date (x : t) : iso_week_date =
    let weekday =
      weekday x
    in
    let week_of_year =
  ...
  let iso_ord_date ({year; day_of_year; _} : t) : iso_ord_date =
    { year; day_of_year }

  let ymd_date ({ year; day_of_year; _ } : t) : ymd =
